...
/// Wraps the bundled mediaremote-adapter helper (Playback/MediaRemoteAdapter/, vendored from
...
final class NowPlayingMonitor {
    ...
    func send(_ command: Command) {
        ...
        do {
            ...
            PiPanelLogger.playback.error("NowPlayingMonitor: failed to send command \(command.rawValue): \(String(describing: error))")
        }
    }
    ...
    private static func resolveAdapterPaths() -> (script: URL, framework: URL)? {
        guard let resourcesRoot = Bundle.main.resourceURL?.appendingPathComponent("MediaRemoteAdapter") else {
            PiPanelLogger.playback.error("NowPlayingMonitor: could not resolve bundle resources URL")
            return nil
        }
        ...
        guard FileManager.default.fileExists(atPath: scriptURL.path),
            ...
            PiPanelLogger.playback.error("NowPlayingMonitor: bundled adapter resources not found at \(resourcesRoot.path)")
            return nil
        }
    ...
    /// removeObserver once the caller no longer needs updates.
    @discardableResult
    ...
    private func launchHelperProcess() {
        ...
        do {
            ...
            debugTrace("playback: NowPlayingMonitor helper process started")
        } catch {
            PiPanelLogger.playback.error("NowPlayingMonitor: failed to launch adapter helper: \(String(describing: error))")
        }
    }
    ...
    private func handleProcessTermination() {
        debugTrace("playback: NowPlayingMonitor helper process terminated")
        process = nil
        stdoutPipe = nil
        current = nil
        notifyObservers()
